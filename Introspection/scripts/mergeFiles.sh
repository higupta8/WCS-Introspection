...
while read line 
do
	 infile="$1/$line""_table.json"
	 if $(hadoop fs -test -d $infile); then echo "Going to copy $infile"; else echo "File $infile does not exist..Moving to next file"; fi

	 outfile="$2/$line""_table.json"
